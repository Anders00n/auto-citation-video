...
####################################################################################################
# Создание видеоклипа с несколькими копиями изображения
####################################################################################################

# Получение продолжительности аудиофайла в секундах
audio_duration = len(final_audio) / 1000

# Создание видеоклипа с несколькими копиями изображения
image_clip = ImageClip("background_cropped.jpg", duration=audio_duration)

# Сохранение видеоклипа
image_clip.write_videofile("background_cropped.mp4", fps=24)
...
image_path = "background_cropped.mp4"
...
ffmpeg_cmd = [
    "ffmpeg",
    "-i",
    image_path,  # Входное изображение (видеоклип)
    "-i",
    audio_path,  # Входное аудио
    "-c:v",
    "libx264",  # Кодек видео
    "-tune",
    "stillimage",  # Оптимизация для изображения (можно убрать для видеоклипа)
    "-c:a",
    "aac",  # Кодек аудио
    "-strict",
